...
def dist_intracluster(df):
    ...
    for i, row1 in df.iterrows():
        for j, row2 in df.iterrows():
            if dist_vect(row1, row2) > d_max:
                d_max = dist_vect(row1, row2)
    return d_max
...
def sep_clusters(df):
    ...
    for i, row1 in df.iterrows():
        for j, row2 in df.iterrows():
            if i != j:
                if dist_vect(row1, row2) < d_min:
                    d_min = dist_vect(row1, row2)
    return d_min
...
def evaluation(chaine, df, df_centres, dico_aff):
    if chaine == "Dunn":
        ...
        return inertie_globale(df, dico_aff)/sep_clusters(df_centres)
